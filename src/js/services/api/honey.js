...
export async function getChapterFrames(chapterUrl) { const html = await fetchMangaHtml(chapterUrl); const doc = parseHtml(html); const pages = extractPages(doc); if (pages.length) return pages; const ajaxPages = await fetchMangaAjaxPages(html, doc); if (ajaxPages.length) return ajaxPages; throw new Error('Сторінки manga.in.ua не завантажилися'); }
