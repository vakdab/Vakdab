...
function renderState(host, state) {
    ...
    const selected = state.animeTitle ? `<div class="live-selection"><strong>${escapeHtml(state.animeTitle)}</strong><span>${state.episodeCount ? `${escapeHtml(state.episodeCount)} ${state.episodeCount === 1 ? 'серія' : 'серій'}` : `Серія ${escapeHtml(state.episode || '—')}`} · ${escapeHtml(state.dub || 'Озвучка не вказана')}</span></div>` : '';
