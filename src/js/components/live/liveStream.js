...
function renderState(host, state) {
    ...
    const selected = state.animeTitle ? `<div class="live-selection"><strong>${escapeHtml(state.animeTitle)}</strong><span>Серія ${escapeHtml(state.episode || '—')} · ${escapeHtml(state.dub || 'Озвучка не вказана')} · ${escapeHtml(state.durationMinutes || '—')} хв</span></div>` : '';
