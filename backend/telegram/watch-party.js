...
async function readState(env) { const raw = await env.MAKIMA_MEMORY?.get(STATE_KEY); if (!raw) return blankState(); try { const parsed = JSON.parse(raw); return { ...blankState(), ...parsed, candidates: Array.isArray(parsed.candidates) ? parsed.candidates.slice(0, MAX_CANDIDATES) : [], animeVotes: parsed.animeVotes && typeof parsed.animeVotes === 'object' ? parsed.animeVotes : {}, dubVotes: parsed.dubVotes && typeof parsed.dubVotes === 'object' ? parsed.dubVotes : {}, anime: parsed.anime && typeof parsed.anime === 'object' ? { ...parsed.anime, dubs: Array.isArray(parsed.anime.dubs) ? parsed.anime.dubs.slice(0, MAX_DUBS) : [] } : null, selection: parsed.selection && typeof parsed.selection === 'object' ? parsed.selection : null, messages: Array.isArray(parsed.messages) ? parsed.messages.slice(-MAX_MESSAGES) : [] }; } catch { return blankState(); } }
...
export async function handleWatchPartyRequest(request, env) {
  ...
  try {
    ...
    if (action === 'vote_anime') {
      ...
      const total = Number(payload.episodesTotal); if (Number.isFinite(total) && total > 0) state.anime.episodesTotal = Math.min(999, Math.max(1, Math.round(total)));
      ...
      if (!admin) return error('Тільки власник може запустити ефір', 403, origin); if (state.status !== 'dub_voting') return error('Спочатку зафіксуй аніме та озвучку', 409, origin); if (!Object.keys(state.dubVotes).length) return error('Спочатку дочекайся голосів за озвучку', 400, origin); const availableDub = state.anime?.dubs?.[0] || 'Озвучка сайту'; state.selection = { time: TIME_OPTIONS[0], dub: winnerFor(countValues(state.dubVotes), availableDub), episodes: 1 }; state.status = 'live'; state.episode = 1; state.position = 0; state.playing = true; state.startedAt = Date.now();
...
export const __watchPartyTest = { kyivDay, daySeed, normalizeAnime, pickDailyCandidates };
