...
Після команди `/start` бот перевіряє підписку користувача на канал [`@vakluna`](https://t.me/vakluna) через Telegram Bot API. Якщо підписка підтверджена, бот показує головне меню; якщо ні — надсилає кнопку переходу до каналу та кнопку повторної перевірки. Для роботи `getChatMember` VakDabBot потрібно додати до каналу адміністратором із правом бачити учасників.
