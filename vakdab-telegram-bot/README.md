...
Команда `/music` або `/shazam` переводить приватний чат у режим пошуку музики. Після цього потрібно надіслати коротке відео або відео як document — Worker передасть файл у AudD, покаже знайдений трек і до п’яти можливих варіантів для вибору. TikTok-посилання, текстові запити та окремі аудіофайли в цьому режимі не обробляються. Для розпізнавання відео потрібен секрет `AUDD_API_TOKEN`; standard endpoint AudD має обмеження 10 MB. Після вибору бот може надіслати доступне коротке офіційне preview та посилання на легальну сторінку треку, але не повну чужу пісню без дозволу правовласника.
