...
Основна кнопка AI тепер називається **«Запитати Луну»**. Підтримуються команди `/luna <запит>` і `/ask <запит>`. Луна підтримує розмови на широке коло тем, зокрема стосунки, флірт, секс і тіло між повнолітніми — без автоматичного моралізаторства. Тему війни, бойових дій та політичного насильства вона не обговорює. Тимчасові помилки або rate limit AI-провайдера повторюються автоматично; якщо сервіс усе ж недоступний, бот відповідає дружнім повідомленням і просить повторити репліку, а не показує технічну помилку. Команда `/luna` активує режим Луни й без аргументів залишає його активним для наступних повідомлень. Команда `/clear` видаляє з Telegram видимі повідомлення користувача та Луни, але не стирає `history`, `summary` або `profile`, тому Луна зберігає контекст і пам’ять. Команда `/makima` та `/ask` тимчасово залишаються backward-compatible alias для старих користувачів. Команда `/memory` показує збережені про користувача факти без AI-запиту, `/forget` очищує історію діалогу, а `/forgetall` — історію та профіль. Memory binding збережено під старою технічною назвою `MAKIMA_MEMORY`, тому існуюче сховище пам’яті не втрачається під час перейменування на Луну.
