...
Основна кнопка AI тепер називається **«Запитати Луну»**. Підтримуються команди `/luna <запит>` і `/ask <запит>`. Луна підтримує розмови на широке коло тем, зокрема стосунки, флірт, секс і тіло між повнолітніми — без автоматичного моралізаторства. Тему війни, бойових дій та політичного насильства вона не обговорює. Команда `/makima` тимчасово залишається backward-compatible alias для старих користувачів, але вона не показується в меню. Команда `/memory` показує збережені про користувача факти без AI-запиту, `/forget` очищує історію діалогу, а `/forgetall` — історію та профіль. Memory binding збережено під старою технічною назвою `MAKIMA_MEMORY`, тому існуюче сховище пам’яті не втрачається під час перейменування на Луну.
