...
Популярні аніме та розклад залишаються окремими anime-only flows. Для аніме доступна кнопка **«Дивитись на VakDab»**, а для манґи й ранобе бот показує деталі та постер із посиланням на Hikka, якщо Telegram не може відкрити внутрішні деталі. Кнопка **«Розклад»** відкриває `schedule.html` як Telegram Mini App: сайт відображає наданий банер, актуальні дані API та читабельні вкладки від понеділка до неділі. Технічна назва джерела даних користувачу не показується.

### Музика

Команда `/music` або `/shazam` переводить приватний чат у режим пошуку музики. Після цього можна надіслати коротке відео, аудіофайл або аудіо/відео як document — Worker передасть файл у AudD і поверне назву, виконавця, альбом та офіційні посилання. TikTok-посилання спочатку обробляються через офіційний oEmbed, а для коротких share-посилань додатково використовується resolver метаданих, після чого аудіо передається в AudD для розпізнавання. Для розпізнавання файлів і TikTok-аудіо потрібен секрет `AUDD_API_TOKEN`; standard endpoint AudD має обмеження 10 MB. Після пошуку бот може надіслати лише коротке офіційне preview з музичного каталогу, а не повну чужу пісню.
...
Під час розмови бот не додає кнопки під пересланими повідомленнями. Команда **`/next`** завершує поточний діалог і відразу починає пошук нового співрозмовника; команда **`/report`** фіксує скаргу та завершує сесію. Є захист від повторних webhook updates, базовий rate limit, блокування посилань/контактів/доксингу та lazy-retention для старих queue, rate-limit і report записів. Це **не повна автоматична модерація**: користувачам не слід надсилати персональні дані чи погоджуватися на небезпечні пропозиції.

### Приватна статистика

Worker зберігає мінімальну технічну статистику взаємодій: Telegram user ID, username, відображуване ім’я, дату першої та останньої активності й кількість взаємодій. Команда **`/F8`** показує до 25 останніх користувачів лише власнику з username `@vaditx`; іншим користувачам вона не розкриває список. Статистика починає накопичуватися лише після деплою цієї версії Worker.
...
AUDD_API_TOKEN                  # розпізнавання аудіо, відео та TikTok
...
Секрети не зберігаються в GitHub і не записуються в код. Додайте ключ до Worker командою `npx wrangler secret put BAZAARLINK_API_KEY` з каталогу `vakdab-telegram-bot`; вставляйте повне значення лише в інтерактивний prompt Wrangler. Не додавайте його до `.env`, комітів, issue або повідомлень чату.
