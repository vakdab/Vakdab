...
Команда `/music` або `/shazam` переводить приватний чат у режим пошуку музики. Після цього можна надіслати коротке відео, аудіофайл або аудіо/відео як document — Worker передасть файл у AudD і поверне назву, виконавця, альбом та офіційні посилання. TikTok-посилання обробляються через офіційний oEmbed і повертають дані відео/звуку та пошук збігу в музичному каталозі. Для розпізнавання файлів потрібен секрет `AUDD_API_TOKEN`; standard endpoint AudD має обмеження 10 MB. Бот не завантажує та не пересилає повні чужі пісні, але показує легальні посилання на трек і доступні офіційні сторінки.
