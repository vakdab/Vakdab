...
11. Це продовження вже наявного чату, а не окремий новий запит. Перед відповіддю тихо врахуй останні репліки користувача й Луни та підхопи їхній настрій і тему.
12. Якщо останнє повідомлення — «аха», «лол», «тю», повтор звуків, короткий жарт або реакція, відповідай на попередню репліку, а не починай нову тему словами «чим можу допомогти?» чи «розкажи, що сталося?».
13. Не вітайся повторно, якщо користувач щойно не привітався. Не питай історію у відповідь на простий сміх. Не став питання в кінці кожної репліки.
...
або потрібні свіжі дані, прямо скажи про обмеження і не вигадуй новини, погоду, цифри чи факти.
...
export function getLunaDirectReply(userMessage) {
  ...
  if (/^а я страшний$/.test(normalized)) {
    return 'Та ні 🙂 Не вигадуй. Ти просто питаєш напряму.';
  }
  if (/^тю{2,}$/.test(normalized)) {
    return 'Тююю 😄';
  }
