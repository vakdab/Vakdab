...
import { fetchTmdbTrendingAnime, fetchTmdbDetail, parseTmdbId } from '../api/tmdb.js';
...
export async function buildHeroBanner() {
            ...
            // TMDB trending першим — кращі постери і метадані
            const [tmdbResult, topResult, mainResult] = await Promise.allSettled([
                fetchTmdbTrendingAnime(),
                fetchAnimeuaTop100(),
                fetchAnimeuaMain(1)
            ]);

            const tmdbTrending = tmdbResult.status === 'fulfilled' ? (tmdbResult.value || []) : [];
            ...
            // TMDB trending має пріоритет — у нього вже є backdrops і описи
            const selectedTmdb = getRandomItems(tmdbTrending, 5);
            const selectedTop = getRandomItems(topAnime, 3);
            heroItems = [...selectedTmdb, ...selectedTop].sort(() => 0.5 - Math.random());
...
export async function loadHeroItemDetails(idx) {
            ...
            try {
                const tmdbId = parseTmdbId(item.url);
                const detail = tmdbId
                    ? await Promise.race([fetchTmdbDetail(tmdbId), timeoutPromise])
                    : await Promise.race([loadAnimeuaDetail(item.url), timeoutPromise]);
            ...
            slide.innerHTML = `
                ...
                <div class="hero-slide-content">
                    ...
                    <div class="hero-slide-tags">
                        ${genres.slice(0, 4).map(g => `<span class="hero-tag genre-tag">${g}</span>`).join('')}
                    </div>
