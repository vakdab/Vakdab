...
import { parseTmdbId, fetchTmdbDetail } from '../api/tmdb.js';
...
export async function openPlayerPage(url) {
            ...
            try {
                // TMDB або AnimeUA джерело
                const tmdbId = parseTmdbId(url);
                let anime;
                if (tmdbId) {
                    anime = await fetchTmdbDetail(tmdbId);
                    // Спробуємо знайти серії на animeua за назвою
                    try {
                        const searchResults = await searchAnimeua(anime.title, 1);
                        if (searchResults && searchResults.length) {
                            const uaDetail = await loadAnimeuaDetail(searchResults[0].url);
                            // Мердж: беремо сезони/серії з animeua, метадані з TMDB
                            if (uaDetail.seasons && Object.keys(uaDetail.seasons).length) {
                                anime.seasons = uaDetail.seasons;
                                anime.sources = uaDetail.sources || ['Основне'];
                                anime.url = uaDetail.url;
                            }
                        }
                    } catch (e) {
                        console.warn('Could not find animeua sources for TMDB title:', e.message);
                    }
                } else {
                    anime = await loadAnimeuaDetail(url);
                }
