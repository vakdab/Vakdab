...
export class LampaPlayer {
  ...
  _init() {
    ...
    controls.innerHTML = `
      <div class="lp-progress-wrap" id="lpProgress"><div class="lp-progress-fill" id="lpProgressFill" style="width:0%"></div></div>
      <div class="lp-bottom-row">
        <button class="lp-btn" id="lpPlayBtn">${LP_ICONS.play}</button>
        <span class="lp-time" id="lpTime">0:00 / 0:00</span>
        <div class="lp-spacer"></div>
        <div class="lp-volume-wrap">
          <button class="lp-btn" id="lpVolBtn">${LP_ICONS.volOn}</button>
          <input type="range" class="lp-volume-slider" id="lpVolSlider" min="0" max="1" step="0.05" value="0.8">
        </div>
        <button class="lp-btn lp-fs-btn" id="lpFsBtn">${LP_ICONS.fsEnter}</button>
      </div>
    `;
  ...
  _bindEvents() {
    ...
    v.addEventListener('play', () => { this.state.playing = true; this._updatePlayBtn(); });
    v.addEventListener('pause', () => { this.state.playing = false; this._updatePlayBtn(); });
    v.addEventListener('timeupdate', () => { this.state.currentTime = v.currentTime; this.state.duration = v.duration || 0; this._updateProgress(); });
    v.addEventListener('waiting', () => { this.state.loading = true; this._spinner.classList.remove('hidden'); });
    v.addEventListener('playing', () => { this.state.loading = false; this._spinner.classList.add('hidden'); });
    v.addEventListener('canplay', () => { this.state.loading = false; this._spinner.classList.add('hidden'); });
    v.addEventListener('error', () => { this.state.loading = false; this._spinner.classList.add('hidden'); });
    v.addEventListener('ended', () => { this.state.playing = false; this._updatePlayBtn(); });
    ...
    if (volSlider) {
      ...
      volSlider.addEventListener('input', () => { v.volume = parseFloat(volSlider.value); this.state.volume = v.volume; this.state.muted = v.volume === 0; this._updateVolBtn(); });
    }
    if (volBtn) volBtn.addEventListener('click', e => { e.stopPropagation(); v.muted = !v.muted; this.state.muted = v.muted; this._updateVolBtn(); });
  ...
  _updateProgress() {
    ...
    const v = this.videoRef;
    if (fill && v && v.duration) fill.style.width = (v.currentTime / v.duration * 100) + '%';
    if (time && v) time.textContent = lpFmtTime(v.currentTime) + ' / ' + lpFmtTime(v.duration);
  }
  _showControls() {
    ...
    if (this.state.playing) this._controlsTimer = setTimeout(() => c.classList.add('hidden'), 3000);
  }
  _flashCenter() {
  ...
  loadSource(src, animeTitle, episodeTitle) {
    if (window.isEmbedUrl && window.isEmbedUrl(src)) {
      this.container.innerHTML = '';
    ...
    const proxyUrl = (typeof window.getProxyUrl === 'function' && !src.startsWith(window.PROXY_URL)) ? window.getProxyUrl(src) : src;

    const _startHls = () => {
      const hls = new Hls({ enableWorker: false, lowLatencyMode: false, backBufferLength: 90, maxBufferLength: 30, xhrSetup: function(xhr) { xhr.withCredentials = false; } });
      this.hls = hls;
      hls.loadSource(proxyUrl);
      hls.attachMedia(v);
      hls.on(Hls.Events.MANIFEST_PARSED, () => { this.state.loading = false; this._spinner.classList.add('hidden'); v.play().catch(() => {}); });
      hls.on(Hls.Events.ERROR, (ev, ed) => {
        if (ed && ed.fatal) { hls.destroy(); this.hls = null; this.state.loading = false; this._spinner.classList.add('hidden'); v.src = proxyUrl; v.load(); v.play().catch(() => {}); }
      });
    };

    if (typeof Hls !== 'undefined' && Hls.isSupported()) _startHls();
      ...
      sc.onload = () => { if (Hls.isSupported()) _startHls(); else { v.src = proxyUrl; v.load(); v.play().catch(() => {}); this.state.loading = false; this._spinner.classList.add('hidden'); } };
  ...
  toggleFullscreen() {
    const target = (this.containerRef && document.body.contains(this.containerRef)) ? this.containerRef : this.container;
    if (!target) return;
