...
const Storage = {
  ...
  getProfile() { try { const raw = localStorage.getItem('vakdab_profile'); return raw ? JSON.parse(raw) : null; } catch { return null; } },
  _setProfile(data) { localStorage.setItem('vakdab_profile', JSON.stringify(data)); },
  setProfile(data) { this._setProfile(data); this._debounceSync(); },

  getHistory() { try { const raw = localStorage.getItem('vakdab_history'); return raw ? JSON.parse(raw) : []; } catch { return []; } },
  _setHistory(h) { localStorage.setItem('vakdab_history', JSON.stringify(h)); },
  setHistory(h) { this._setHistory(h); this._debounceSync(); },

  getBookmarks() { try { const raw = localStorage.getItem('vakdab_bookmarks'); return raw ? JSON.parse(raw) : []; } catch { return []; } },
  _setBookmarks(b) { localStorage.setItem('vakdab_bookmarks', JSON.stringify(b)); },
  setBookmarks(b) { this._setBookmarks(b); this._debounceSync(); },

  getLikes() { try { const raw = localStorage.getItem('vakdab_likes'); return raw ? JSON.parse(raw) : {}; } catch { return {}; } },
  _setLikes(l) { localStorage.setItem('vakdab_likes', JSON.stringify(l)); },
  setLikes(l) { this._setLikes(l); this._debounceSync(); },

  getWatchTime() { try { const raw = localStorage.getItem('vakdab_watchTime'); return raw ? parseInt(raw, 10) : 0; } catch { return 0; } },
  _setWatchTime(t) { localStorage.setItem('vakdab_watchTime', String(t)); },
  addWatchTime(seconds) { const total = this.getWatchTime() + seconds; this._setWatchTime(total); this._debounceSync(); return total; },
