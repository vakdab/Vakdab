...
import { fetchTmdbAnime, fetchTmdbTopAnime, searchTmdbAnime } from '../api/tmdb.js';
...
export async function fetchContent() {
            if (currentTab === 'top100') {
                // TMDB топ + animeua топ — мердж для кращого покриття
                try {
                    const tmdbTop = await fetchTmdbTopAnime(currentPage);
                    if (tmdbTop.length) return tmdbTop;
                } catch (e) { console.warn('TMDB top failed, falling back:', e.message); }
                return await fetchAnimeuaTop100();
            }
            if (currentSearchQuery) {
                try {
                    const tmdbResults = await searchTmdbAnime(currentSearchQuery, currentPage);
                    if (tmdbResults.length) return tmdbResults;
                } catch (e) { console.warn('TMDB search failed, falling back:', e.message); }
                return await searchAnimeua(currentSearchQuery, currentPage);
            }
            if (currentCategory) {
                return await fetchAnimeuaByCategory(currentCategory, currentPage);
            }
            // За замовчуванням — TMDB discover (популярне аніме)
            try {
                const tmdbList = await fetchTmdbAnime(currentPage);
                if (tmdbList.length) return tmdbList;
            } catch (e) { console.warn('TMDB main failed, falling back to animeua:', e.message); }
