...
test('main bot menu opens the Shazam Mini App', () => {
  ...
  assert.match(worker, /WATCH_PARTY_WEB_APP_URL = 'https:\/\/vakdab\.animegran8\.workers\.dev\/app\/watch-party\?v=20260825-watchparty-v13'/);
...
test('Watch Party uses multi-anime vote, separate dub vote and locked continuation', () => {
  assert.match(watchPartyHtml, /animeSiteLink/);
  assert.match(watchPartyHtml, /id="startButton"[^>]*>Почати ефір/);
  assert.match(watchPartyJs, /\$\('startButton'\)\.hidden = status !== 'dub_voting'/);
  assert.match(watchPartyHtml, /id="startHint"/);
  ...
  assert.match(watchPartyJs, /if\(typeof raw === 'number'\) value = raw/);
  assert.match(watchPartyJs, /normalizeEpisodeNumber/);
  assert.match(watchPartyJs, /function renderChat/);
  assert.match(watchPartyJs, /function attachVideoSource/);
  assert.match(watchPartyHtml, /hls\.js@1\.5\.17/);
  assert.match(watchPartyHtml, /Без голосів обереться перша доступна озвучка/);
